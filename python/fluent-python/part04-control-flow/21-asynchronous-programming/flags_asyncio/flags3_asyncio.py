...
async def get_country(client: httpx.AsyncClient,
                      base_url: str,
                      cc: str) -> str:    # This coroutine returns a string with the country name—if all goes well
    url = f'{base_url}/{cc}/metadata.json'.lower()
    resp = await client.get(url, timeout=3.1, follow_redirects=True)
    resp.raise_for_status()
    metadata = resp.json()  # metadata will get a Python dict built from the JSON contents of the response
...
async def download_one(client: httpx.AsyncClient,
    ...
    try:
        ...
        # we use 2 `with` blocks controlled by `semaphore` since it's good 
        # practice to hold semaphores and locks for the shortest possible time
